...
DATA_PATH = os.path.join(ROOT_DIR, 'data','BIM')
g_classes = [x.rstrip() for x in open(os.path.join(BASE_DIR, 'meta/class_names.txt'))]
g_class2label = {cls: i for i,cls in enumerate(g_classes)}

colors = cc.glasbey[:len(g_classes)]
colors = np.array([matplotlib.colors.hex2color(color) for color in colors])
colors = (colors * 255).astype(int)

# g_class2color = {}
# for i in range(len(colors)):
#     g_class2color[g_classes[i]] = list(colors[i])
